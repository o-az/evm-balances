import type { Balance, Chain, Token } from '@/types';
import { DEPLOYED_CONTRACTS } from '@/chain/constants';
import { evmRpcBatchRequest, evmRpcRequest } from '@/chain/rpc';
import { POSSIBLE_CHAIN_NAMES, type PossibleChainNames, mapChain } from './mapper';
...
export const getBatchLimit = (chain: Chain) =>
  chain === 'ethereum' ? 1000 : Number.POSITIVE_INFINITY;
...
export async function getTokensBalances({
  ...
  try {
    ...
    if (!POSSIBLE_CHAIN_NAMES.includes(chain)) {
    ...
    const { default: tokenList } = (await import(`./data/tokens/${_chain}.json`, {
      assert: { type: 'json' },
    })) as {
      default: Token;
    };
    ...
    if (tokenAddresses.length > batchLimit) {
      ...
      const dataArray = chunks.map(tokens =>
        buildCallData({ method: METHOD_HASH_SIGNATURE, address, tokens })
      );
      const response = await evmRpcBatchRequest({ chain: mappedChain, contract, dataArray });
      const outOfGas = response
        .filter(({ error }) => error?.code === -32_000)
        .map(({ error }) => error);
    ...
    if (error || !result)
      return { balances: [], error: error?.message ?? 'No result from RPC request' };
...
function parseResult(result: string, tokenAddresses: string[], tokens: Token) {
  ...
  for (const [index, tokenAddress] of tokenAddresses.entries()) {
    ...
    const balance = Number.parseInt(chunks[index], 16) / Math.pow(10, decimals);
    if (!balance) continue;
    balances.push({ ...token, balance, address: tokenAddress });
  }
  return balances;
}
